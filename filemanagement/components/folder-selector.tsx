...
const FolderSelector: React.FC<FolderSelectorProps> = ({ onFolderExistenceChange }) => {
 ...
 // --- Handler Pemilihan Workspace ---
   ...
   // --- Memuat Workspace & Auto-Select ---
    const loadWorkspaces = useCallback(async (selectFirst = false) => {
        if (!user?.id || !accessToken) { setIsLoadingWorkspaces(true); return; }
        setIsLoadingWorkspaces(true); setWorkspaceError(null);
        try {
            const { data: supabaseWorkspaces, error: supabaseError } = await supabase.from('workspace').select('id, user_id, url, name, color').eq('user_id', user.id);
            if (supabaseError) throw new Error(`Supabase Error: ${supabaseError.message}`);
            const loadedWorkspaces = (supabaseWorkspaces as Workspace[]) || [];
            setWorkspaces(loadedWorkspaces);
            const shouldAutoSelect = selectFirst || !selectedWorkspaceForBrowse;
            if (shouldAutoSelect && loadedWorkspaces.length > 0) {
                 const currentSelectionExists = selectedWorkspaceForBrowse && loadedWorkspaces.some(ws => ws.id === selectedWorkspaceForBrowse.id);
                 if (!currentSelectionExists) { handleSelectWorkspaceForBrowse(loadedWorkspaces[0]); }
            } else if (loadedWorkspaces.length === 0) {
                setSelectedWorkspaceForBrowse(null); setCurrentFolderId(null); setItemsInCurrentFolder([]); setFolderPath([]);
                if (onFolderExistenceChange) onFolderExistenceChange(false); // Panggil false jika tak ada workspace
            }
        } catch (err: any) {
    ...
    }, [user?.id, accessToken, supabase, selectedWorkspaceForBrowse, handleSelectWorkspaceForBrowse, onFolderExistenceChange]); // Tambahkan onFolderExistenceChange

    // Trigger loadWorkspaces saat user/token siap
    useEffect(() => {
        if (user?.id && accessToken) { loadWorkspaces(true); }
