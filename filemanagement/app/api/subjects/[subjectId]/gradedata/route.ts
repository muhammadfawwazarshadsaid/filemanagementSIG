...
export async function GET(
    ...
    console.log(`[API GET /api/subjects/${subjectId}/gradedata] Fetching grade data from backend...`);

    if (!BACKEND_URL) { /* ... (handle error BACKEND_URL not set) ... */ return NextResponse.json({ message: 'Konfigurasi backend URL tidak ditemukan' }, { status: 500 }); }
    if (!subjectId || subjectId === 'unknown') { return NextResponse.json({ message: 'Subject ID tidak valid di URL' }, { status: 400 }); }

    try {
        // ------ Panggil Backend (GET) ------
        const authToken = request.headers.get('Authorization'); // (Placeholder) Ambil token
        const backendEndpoint = `${BACKEND_URL}/subjects/${subjectId}/grades`; // Contoh endpoint (SESUAIKAN!)
        ...
        const response = await fetch(backendEndpoint, {
            ...
            // cache: 'no-store', // Pertimbangkan cache
        });

        if (!response.ok) {
            let errorBody; try { errorBody = await response.json(); } catch {/**/}
            console.error(`[API GET /api/subjects/${subjectId}/gradedata] Backend error (${response.status}):`, errorBody?.message || response.statusText);
            throw new Error(errorBody?.message || `Gagal mengambil data nilai dari backend (${response.status})`);
        }

        // Asumsi backend mengembalikan struktur: { students, assessmentComponents, subjectName, initialGrades }
        const backendData = await response.json();

        // (Opsional) Mapping data jika perlu

        console.log(`[API GET /api/subjects/${subjectId}/gradedata] Data fetched from backend.`);
        return NextResponse.json(backendData, { status: 200 });
        // ------ Akhir Panggil Backend (GET) ------

    } catch (error) {
        console.error(`[API GET /api/subjects/${subjectId}/gradedata] Error:`, error);
        const message = error instanceof Error ? error.message : 'Kesalahan Server Internal';
        return NextResponse.json({ message: message.startsWith('Gagal') ? message : 'Kesalahan Server Internal' }, { status: 500 });
    }
}

// --- Handler POST (Simpan Nilai ke Backend) ---
export async function POST(
    request: NextRequest,
    context: { params: { subjectId: string } }
) {
    const { subjectId } = context.params;
     console.log(`[API POST /api/subjects/${subjectId}/gradedata] Saving grade via backend...`);
    ...
    try {
        ...
        // Validasi input frontend (tetap lakukan)
        if (studentId === undefined || componentId === undefined || score === undefined) { return NextResponse.json({ message: 'Data nilai tidak lengkap' }, { status: 400 }); }
        const finalScore = score === null || score === '' ? null : Number(score);
        if (finalScore !== null && (isNaN(finalScore) || finalScore < 0 || finalScore > 100)) { return NextResponse.json({ message: 'Nilai harus 0-100 atau kosong' }, { status: 400 }); }

        // ------ Panggil Backend (POST) ------
        const authToken = request.headers.get('Authorization'); // (Placeholder) Ambil token
        // Tentukan endpoint backend untuk menyimpan nilai (SESUAIKAN!)
        // Opsi 1: Endpoint spesifik per subject
        // const backendEndpoint = `${BACKEND_URL}/subjects/${subjectId}/grades`;
        // Opsi 2: Endpoint umum untuk menyimpan nilai
        const backendEndpoint = `${BACKEND_URL}/grades`; // Contoh

        // Siapkan payload untuk backend (mungkin perlu subjectId juga?)
        const backendPayload = {
            studentId,
            componentId,
            score: finalScore,
            subjectId: subjectId // (Opsional, jika backend memerlukannya)
        };
        ...
        if (!response.ok) {
            let errorBody; try { errorBody = await response.json(); } catch {/**/}
             console.error(`[API POST /api/subjects/${subjectId}/gradedata] Backend error (${response.status}):`, errorBody?.message || response.statusText);
             // Teruskan pesan error dari backend jika ada dan aman
            throw new Error(errorBody?.message || `Gagal menyimpan nilai ke backend (${response.status})`);
        }

        // Ambil respons dari backend (jika ada data penting)
        const backendResponse = await response.json();

        console.log(`[API POST /api/subjects/${subjectId}/gradedata] Grade saved via backend. Backend response:`, backendResponse);

        // Kirim kembali pesan sukses (atau data dari backend jika perlu)
        return NextResponse.json({ message: backendResponse?.message || `Nilai berhasil disimpan.` }, { status: 200 });
        // ------ Akhir Panggil Backend (POST) ------

    } catch (error: any) {
        if (error instanceof SyntaxError) { /* ... handle invalid JSON ... */ return NextResponse.json({ message: 'Format JSON tidak valid' }, { status: 400 });}
        console.error(`[API POST /api/subjects/${subjectId}/gradedata] Error:`, error);
        const message = error instanceof Error ? error.message : 'Kesalahan Server Internal';
        return NextResponse.json({ message: message.startsWith('Gagal') ? message : 'Kesalahan Server Internal' }, { status: 500 });
    }
