...
export default function InputNilaiPage() {
    ...
    const fetchInitialData = useCallback(async () => {
        ...
        try {
            ...
            const apiUrl = `/api/gradedata?subjectId=${subjectId}`;
    ...
    // --- Callback Simpan Satu Nilai (Untuk DataTable) ---
     const handleSaveSingleGradeClient = useCallback(async (studentId: string, componentId: string, score: number | null): Promise<void> => {
        if (!subjectId) { throw new Error("ID Mata Pelajaran tidak valid."); } // Cek subjectId state
        try {
             console.log(`[page.tsx Save] Sending: St:${studentId} Comp:${componentId} Score:${score} Subj:${subjectId}`);
             const response = await fetch(`/api/gradedata?subjectId=${subjectId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId, componentId, score }), });
             const result = await response.json(); console.log(`[page.tsx Save] Response status: ${response.status}`, result);
             if (!response.ok) { throw new Error(result.message || `Gagal menyimpan (${response.status})`); }
             setGradeData(prevData => { if (!prevData) return null; const newInitialGrades = structuredClone(prevData.initialGrades); if (!newInitialGrades[studentId]) { newInitialGrades[studentId] = {}; } newInitialGrades[studentId][componentId] = score; console.log(`[page.tsx Save] Successfully updated state for ${studentId}.`); return { ...prevData, initialGrades: newInitialGrades }; });
        } catch (err) { console.error('[page.tsx Client Save] Error:', err); throw err; }
     }, [subjectId]); // Dependensi ke subjectId (dari state)
