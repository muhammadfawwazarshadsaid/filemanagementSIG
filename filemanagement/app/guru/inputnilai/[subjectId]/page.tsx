...
import { Loader2, PlusCircle, Scale, Trash2, Settings, Save, Pencil, XCircle, Download } from 'lucide-react';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";

// Impor jsPDF dan autoTable
import jsPDF from 'jspdf';
// Impor tipe spesifik dari jspdf-autotable
...
// --- Tipe Data --- (Impor dari schema atau definisikan di sini)
...
export default function InputNilaiPage() {
    ...
    const saveInlineEdit = async () => {
        if (!editingComponentId || !subjectId) return;
        const currentComponent = editableComponents.find(c => c.id === editingComponentId); if (!currentComponent) return;
        const { name, weight } = editingValues; if (!name?.trim() || !weight?.trim()) { setDialogError("Nama/Bobot kosong."); return; } const weightValue = parseFloat(weight); if (isNaN(weightValue) || weightValue <= 0) { setDialogError("Bobot > 0."); return; } if (name.trim() === currentComponent.name && weightValue === currentComponent.weight) { cancelInlineEdit(); return; }
        setIsDialogLoading(true); setDialogError(null);
        try {
             const response = await fetch(`/api/subjects/${subjectId}/components/${editingComponentId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim(), weight: weightValue }), });
             const result = await response.json(); if (!response.ok) throw new Error(result.message || `Gagal (${response.status})`);
             await handleUpdateComponent(result as AssessmentComponent);
             cancelInlineEdit(); toast.success(`Komponen "${result.name}" diubah.`);
        } catch (err) { console.error("[Dialog] Edit component error:", err); const msg = err instanceof Error ? err.message : "Gagal ubah"; setDialogError(msg); toast.error(msg); } finally { setIsDialogLoading(false); }
     };
    const handleDialogAddComponent = async () => {
        if (!dialogNewComponentName.trim() || !dialogNewComponentWeight.trim()) { setDialogError("Nama/Bobot baru kosong."); return; } const weightValue = parseFloat(dialogNewComponentWeight); if (isNaN(weightValue) || weightValue <= 0) { setDialogError("Bobot baru > 0."); return; }
        setIsDialogLoading(true); setDialogError(null);
        try {
            const response = await fetch(`/api/subjects/${subjectId}/components`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: dialogNewComponentName.trim(), weight: weightValue }), });
            const result = await response.json(); if (!response.ok) throw new Error(result.message || `Gagal (${response.status})`);
            const newComponent: AssessmentComponent = result;
            setGradeData(prevData => { if (!prevData) return null; const updatedComponents = [...prevData.assessmentComponents, newComponent]; const updatedGrades = structuredClone(prevData.initialGrades); Object.keys(updatedGrades).forEach(studentId => { updatedGrades[studentId] = { ...updatedGrades[studentId], [newComponent.id]: null }; }); setEditableComponents(updatedComponents); return { ...prevData, assessmentComponents: updatedComponents, initialGrades: updatedGrades }; });
            setDialogNewComponentName(''); setDialogNewComponentWeight(''); toast.success(`Komponen "${newComponent.name}" ditambah.`);
        } catch (err) { console.error("[Dialog] Add component error:", err); const msg = err instanceof Error ? err.message : "Gagal tambah"; setDialogError(msg); toast.error(msg); } finally { setIsDialogLoading(false); }
    };
    ...
    const confirmDeleteComponent = async () => {
        ...
        try {
            ...
            const result = await response.json(); if (!response.ok) throw new Error(result.message || `Gagal (${response.status})`);
            const deletedId = deleteDialogState.componentId; const deletedName = deleteDialogState.componentName;
            setGradeData(prevData => { if (!prevData) return null; const updatedComponents = prevData.assessmentComponents.filter(c => c.id !== deletedId); const updatedGrades = structuredClone(prevData.initialGrades); Object.keys(updatedGrades).forEach(studentId => { if (updatedGrades[studentId]?.[deletedId] !== undefined) { delete updatedGrades[studentId][deletedId]; } }); setEditableComponents(updatedComponents); return { ...prevData, assessmentComponents: updatedComponents, initialGrades: updatedGrades }; });
            closeDeleteDialog();
            toast.success(`Komponen "${deletedName}" dihapus.`);
        } catch (err) { console.error("[Dialog] Delete component error:", err); const msg = err instanceof Error ? err.message : "Gagal hapus"; setDeleteDialogState(prev => ({ ...prev, isLoading: false, error: msg })); toast.error(msg); }
    };
    ...
    // --- Fungsi Inti Generate Dokumen PDF ---
     const generatePdfDocument = useCallback((orientation: PdfOrientation, paperSize: PdfPaperSize): jsPDF | null => {
        ...
        doc.setFontSize(16); doc.setFont("helvetica", "bold"); doc.text(`Rekap Nilai - ${subjectName}`, doc.internal.pageSize.getWidth() / 2, 15, { align: 'center' });
        doc.setFontSize(10); doc.setFont("helvetica", "normal"); doc.setTextColor(100); doc.text(`Lokasi: ${location}`, 14, 22); doc.text(`Tanggal Cetak: ${generatedDate}`, 14, 27);
        const head: CellDef[][] = [[
            { content: 'No', styles: { halign: 'center' as const, fontStyle: 'bold' as const } },
            { content: 'Nama Siswa', styles: { fontStyle: 'bold' as const } },
            { content: 'Kelas', styles: { fontStyle: 'bold' as const, halign: 'center' as const } },
            ...assessmentComponents.map(comp => ({ content: `${comp.name}\n(${comp.weight}%)`, styles: { halign: 'center' as const, fontStyle: 'bold' as const } })),
        ...
        const body: RowInput[] = students.map((student, index) => {
            ...
            assessmentComponents.forEach(comp => { const score = initialGrades[student.id]?.[comp.id] ?? 0; if (typeof score === 'number' && !isNaN(score)) { finalScoreNum += (score * comp.weight); weightSum += comp.weight; } });
            const finalScore = weightSum > 0 ? (finalScoreNum / weightSum).toFixed(1) : '-';
            return [ { content: index + 1, styles: { halign: 'center' as const } }, student.name, { content: student.class, styles: { halign: 'center' as const } }, ...assessmentComponents.map(comp => ({ content: initialGrades[student.id]?.[comp.id]?.toString() ?? '-', styles: { halign: 'center' as const } })), { content: finalScore, styles: { halign: 'center' as const, fontStyle: 'bold' as const } } ];
        });
        const numComponentCols = assessmentComponents.length;
        const fixedWidths = (orientation === 'l' ? 8 : 10) + 15 + (orientation === 'l' ? 16 : 18);
        const nameWidth = 60;
        const availableWidth = doc.internal.pageSize.getWidth() - 28 - fixedWidths - nameWidth;
        ...
        autoTable(doc, {
            ...
            headStyles: { fillColor: [41, 128, 185], textColor: 255, fontStyle: 'bold' as FontStyle, halign: 'center' as HAlignType, fontSize: 9 },
            styles: { fontSize: 8.5, cellPadding: 2, overflow: 'linebreak' },
            columnStyles: { 0: { cellWidth: orientation === 'l' ? 8 : 10, halign: 'center' as HAlignType }, 1: { cellWidth: nameWidth }, 2: { cellWidth: 15, halign: 'center' as HAlignType }, ...assessmentComponents.reduce((acc, comp, idx) => { acc[idx + 3] = { cellWidth: componentColWidth, halign: 'center' as HAlignType }; return acc; }, {} as { [key: number]: { cellWidth?: number; halign?: HAlignType } }), [head[0].length - 1]: { cellWidth: orientation === 'l' ? 16 : 18, halign: 'center' as HAlignType, fontStyle: 'bold' as FontStyle }, },
            didDrawPage: (data) => { doc.setFontSize(8); doc.setTextColor(150); const pageCount = (doc.internal as any).pages.length ? (doc.internal as any).pages.length -1 : doc.getNumberOfPages(); doc.text('Halaman ' + data.pageNumber + ' dari ' + pageCount, data.settings.margin.left, doc.internal.pageSize.getHeight() - 10); }
        });
        return doc;
    }, [gradeData]);

    // --- Fungsi Generate Preview (Dipanggil oleh useEffect) ---
    const handleGeneratePreview = useCallback(async () => {
        ...
        try {
            ...
            if (doc) { setPdfPreviewUrl(doc.output('datauristring')); } else { throw new Error("Gagal buat dokumen PDF."); }
        } catch (pdfError) { console.error("[PDF Preview] Error:", pdfError); toast.error("Gagal buat pratinjau."); setPdfPreviewUrl(null); }
        finally { setIsPreviewLoading(false); }
    }, [gradeData, pdfOrientation, pdfPaperSize, generatePdfDocument, isPdfOptionsDialogOpen]);

    // --- useEffect untuk Pratinjau Otomatis ---
    useEffect(() => {
    ...
    // --- Fungsi Unduh PDF Final ---
    const handleActualDownload = useCallback(() => {
        if (!gradeData) { toast.error("Data belum dimuat."); return; } setIsDownloadingPdf(true);
        setTimeout(() => { try { const doc = generatePdfDocument(pdfOrientation, pdfPaperSize); if (doc) { const { subjectName } = gradeData; const safeSubjectName = subjectName.replace(/[^a-zA-Z0-9]/g, '_'); const timestamp = new Date().toISOString().slice(0,10); const fileName = `Rekap_Nilai_${safeSubjectName}_${timestamp}_${pdfOrientation}_${pdfPaperSize}.pdf`; doc.save(fileName); toast.success("Unduhan PDF dimulai..."); setIsPdfOptionsDialogOpen(false); } else { throw new Error("Gagal buat dokumen PDF."); } } catch (pdfError) { console.error("[PDF Download] Error:", pdfError); toast.error("Gagal mengunduh PDF."); } finally { setIsDownloadingPdf(false); } }, 50);
    ...
    return (
        <div className="container mx-auto p-4 space-y-6">
            {/* Header Halaman dan Tombol Aksi Global */}
            <div className="flex justify-between items-center mb-3 flex-wrap gap-3 p-4 border rounded-md bg-card text-card-foreground shadow-sm">
                <div className={`text-sm font-medium flex items-center gap-1 ${totalWeight !== 100 ? 'text-yellow-600' : 'text-muted-foreground'}`}>
                    <Scale className="h-4 w-4" />
                    Total Bobot: {totalWeight.toFixed(2)}% {totalWeight !== 100 && <span className="font-semibold">(Perhatian: Tidak 100%)</span>}
                </div>
                        ...
                        <DialogContent className="sm:max-w-3xl">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-4">
                                ...
                                <div className="space-y-4 md:col-span-1">
                                    ...
                                    <div className='pt-4'> <Button onClick={handleActualDownload} disabled={isDownloadingPdf || isPreviewLoading} className="w-full"> {isDownloadingPdf ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />} Unduh PDF Sekarang </Button> </div>
                                </div>
                                {/* Preview */}
                                <div className="md:col-span-2 border rounded-md bg-muted/30 min-h-[400px] flex items-center justify-center relative overflow-hidden">
                                    ...
                                    {pdfPreviewUrl ? ( <iframe src={pdfPreviewUrl} className="w-full h-[500px] md:h-[600px]" title="Pratinjau PDF" aria-label="Pratinjau Dokumen PDF"/> ) : ( <div className="text-center text-muted-foreground p-4"><p>Pratinjau akan muncul di sini...</p>{!isPreviewLoading && <p className='text-xs mt-1'>(Mengubah opsi akan memuat ulang)</p>}</div> )}
                                </div>
                            </div>
