...
export default function InputNilaiPage() {
    ...
    // --- Fetch Data Awal ---
    const fetchInitialData = useCallback(async () => { if (!subjectId) return; setIsLoading(true); setError(null); try { const response = await fetch(`/api/subjects/${subjectId}/gradedata`); if (!response.ok) { let errData; try { errData = await response.json(); } catch {} throw new Error(errData?.message || `Error ${response.status}`); } const data: GradeData = await response.json(); data.students.sort((a, b) => a.name.localeCompare(b.name)); setGradeData(data); setEditableComponents(data.assessmentComponents); } catch (err) { setError(err instanceof Error ? err.message : "Gagal memuat data"); setGradeData(null); setEditableComponents([]); } finally { setIsLoading(false); } }, [subjectId]);
    useEffect(() => { fetchInitialData(); }, [fetchInitialData]);

    // --- Callback Simpan Satu Nilai ---
    const handleSaveSingleGradeClient = useCallback(async (studentId: string, componentId: string, score: number | null): Promise<void> => { if (!subjectId) { throw new Error("ID Mata Pelajaran tidak valid."); } try { const response = await fetch(`/api/subjects/${subjectId}/gradedata`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId, componentId, score }), }); const result = await response.json(); if (!response.ok) { throw new Error(result.message || `Gagal menyimpan (${response.status})`); } setGradeData(prevData => { if (!prevData) return null; const newInitialGrades = structuredClone(prevData.initialGrades); if (!newInitialGrades[studentId]) newInitialGrades[studentId] = {}; newInitialGrades[studentId][componentId] = score; return { ...prevData, initialGrades: newInitialGrades }; }); } catch (err) { console.error('[Client Save] Error:', err); throw err; } }, [subjectId]);

    // --- Callback Update Komponen ---
    const handleUpdateComponent = useCallback(async (updatedComponent: AssessmentComponent): Promise<void> => { setGradeData(prevData => { if (!prevData) return null; const updatedComponents = prevData.assessmentComponents.map(c => c.id === updatedComponent.id ? updatedComponent : c ); setEditableComponents(updatedComponents); return { ...prevData, assessmentComponents: updatedComponents }; }); }, []);
    ...
    const overallAverageScore = useMemo(() => {
        ...
        let totalFinalScoreSum = 0; let validStudentCount = 0;
        students.forEach(student => {
            let studentScoreSum = 0; let studentWeightSum = 0;
            assessmentComponents.forEach(comp => { const score = initialGrades[student.id]?.[comp.id]; if (typeof score === 'number' && !isNaN(score) && comp.weight > 0) { studentScoreSum += (score * comp.weight); studentWeightSum += comp.weight; } });
            if (studentWeightSum > 0) { const finalScore = studentScoreSum / studentWeightSum; if (!isNaN(finalScore)) { totalFinalScoreSum += finalScore; validStudentCount++; } }
        });
    ...
    return (
        <div className="container mx-auto p-4 space-y-6">
            {/* Header Halaman (TANPA background/border/shadow) */}
            {/* !! MODIFIKASI className di div ini !! */}
            <div className="flex justify-between items-start mb-3 flex-wrap gap-x-6 gap-y-3 p-4">
                {/* Info Mapel, Tahun Ajaran, Bobot, Rata-rata */}
                <div className='space-y-1'>
                     ...
                     <div className={`text-sm font-medium flex items-center gap-1 ${totalWeight !== 100 ? 'text-yellow-600' : 'text-muted-foreground'}`}>
                        ...
                        Total Bobot: {totalWeight.toFixed(2)}% {totalWeight !== 100 && <span className="font-semibold">(Tidak 100%)</span>}
                     </div>
                     <div className="text-sm font-medium flex items-center gap-1 text-muted-foreground">
                         <Sigma className="h-4 w-4" />
                         Rata-rata Keseluruhan: {overallAverageScore !== null ? overallAverageScore.toFixed(1) : '-'}
                     </div>
                </div>
                ...
                <div className='flex items-center gap-2 flex-shrink-0 self-start pt-1'>
                    ...
                    <Dialog open={isManageDialogOpen} onOpenChange={setIsManageDialogOpen}>
                         {/* ... (Trigger & Content Dialog Atur Komponen - sama) ... */}
